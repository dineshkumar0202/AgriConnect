    ...
    <section className="max-w-6xl mx-auto px-4 pb-10 space-y-6">
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <form
          ...
          className="p-6 rounded-2xl shadow-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 space-y-4 transform transition-all duration-300 hover:shadow-2xl animate-fade-up"
        >
          <h3 className="text-lg font-semibold">{strings.createPost}</h3>

          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs font-semibold">{strings.productType}</label>
              <select
                name="type"
                value={form.type}
            ...
            <div>
              <label className="text-xs font-semibold">{strings.availableDate}</label>
              <input
                type="date"
                name="availableDate"
          ...
          <button className="w-full py-3 rounded-xl bg-green-600 hover:bg-green-500 text-white font-semibold transition-all shadow-md">
        ...
        <div className="space-y-4 animate-fade-up">
          ...
          <div className="space-y-3 max-h-[500px] overflow-y-auto pr-1">
            {myPosts.map((p) => (
              <div
                key={p._id}
                className="p-4 rounded-2xl shadow-md bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 flex items-center justify-between gap-4 hover:shadow-xl transition-all animate-slide-up"
              >
                ...
                <div className="flex flex-col gap-2">
                  <button
                    ...
                    className="px-3 py-1 rounded-full text-xs border border-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800"
                  >
                    {strings.toggleActive}
                  </button>
                  <button
                    onClick={() => deletePost(p._id)}
                    className="px-3 py-1 rounded-full text-xs bg-red-500 text-white hover:bg-red-400"
                  >
                    {strings.delete}
                  </button>
                </div>
